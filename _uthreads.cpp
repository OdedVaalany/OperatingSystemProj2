...
int setup_thread(int tid, char *stack, thread_entry_point entry_point){
    // initializes env[tid] to use the right stack, and to run from the function 'entry_point', when we'll use
    // siglongjmp to jump into the thread.
    ThreadCard* tmp = (ThreadCard*) malloc(sizeof(ThreadCard));
    if(tmp == NULL) return -1;
    threads[tid] = tmp;
    address_t sp = (address_t) threads[tid]->sp + STACK_SIZE - sizeof(address_t);
    address_t pc = (address_t) entry_point;
    sigsetjmp(threads[tid]->_sigjmp_buf, 1);
    ((threads[tid]->_sigjmp_buf)->__jmpbuf)[JB_SP] = translate_address(sp);
    ((threads[tid]->_sigjmp_buf)->__jmpbuf)[JB_PC] = translate_address(pc);
